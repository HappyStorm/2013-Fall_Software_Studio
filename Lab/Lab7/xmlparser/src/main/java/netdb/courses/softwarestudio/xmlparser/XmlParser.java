...
public class XmlParser 
{
	...
	public XmlNode xmlToTree(XmlNode now) throws Exception 
	{
		...
		while(readCursor < content.length() - 1) 
			...
			xmlToTree(now.children.get(now.children.size() - 1));
		}
	 ...
	 */
	public void addTag(String tags, XmlNode now) 
	{
		...
		/*if(now.children.size()<=0)
			System.out.println("Now, this node has no child.!!!");*/
		/*for(int i=0 ; i<now.children.size() ; i++)
			{
				if(now.children.size()<=0)
					System.out.println("Now, this node has no child.!!!");
				else
					System.out.println(now.children.get(i));
			}
		System.out.println("-----------------");*/
		for(int i=1 ; i<atts.length ; i++)
			{
				String[] subString = atts[i].split("=");
				//System.out.println(now.children.get(now.children.size()-1));
				now.children.get(now.children.size()-1).addAttribute(subString[0], subString[1]);
			}
	}
