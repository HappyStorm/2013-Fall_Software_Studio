...
public class LotteryLab {
	...
	public static void main(String[] args) throws Exception 
	{
		...
		//System.out.printf("%d %d %d", from, to, count);
		/**
		 * TODO
		...
		while(!(inputString = reader.readLine()).equals("quit")){
			...
			String[] lotterystring = inputString.split(" ");
			
			for(int i=0 ; i<6 ; i++)
			{
				int a = Integer.parseInt(lotterystring[i]);
				lottery[i] = a;
			}
			
			LotteryTicket gogo = new LotteryTicket();
            
			gogo.ticket = lottery;
			
			gogo.raffle(winningNumbers);
